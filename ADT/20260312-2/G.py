...
def solve():
    ...
    DP = [[INF, INF] for i in range(len(S))]
    # [OFF, ON]
    DP[0][0] = X if S[0] == "a" else Y
    DP[0][1] = Z + X if S[0] == "A" else Z + Y
    for i in range(1, len(S)):
        s = S[i]
        if s == "a":
            DP[i][0] = min(DP[i-1][0] + X, DP[i][0])
            DP[i][0] = min(DP[i-1][1] + Z + X, DP[i][0])
            DP[i][1] = min(DP[i-1][1] + Y, DP[i][1])
            DP[i][1] = min(DP[i-1][0] + Z + Y, DP[i][1])
        else:
            DP[i][0] = min(DP[i-1][0] + Y, DP[i][0])
            DP[i][0] = min(DP[i-1][1] + Z + Y, DP[i][0])
            DP[i][1] = min(DP[i-1][1] + X, DP[i][1])
            DP[i][1] = min(DP[i-1][0] + Z + X, DP[i][1])

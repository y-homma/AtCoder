...
def solve():
    ...
    for k, l in rem.items():
        ...
        if k == 1:
            for item in l:
                if item == 0: ans += nonOne
                if item > 1 and item in rem.keys(): ans += len(rem[item])
    print(ans)

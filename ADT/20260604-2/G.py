...
def solve():
    ...
    A = [input().strip("\n") for _ in range(H)]
    INF = 10 ** 10
    D = [[[INF, INF] for _ in range(W)] for h in range(H)]
    Start = (-1, -1)
    Goal = (-1, -1)
    M = [(-1, 0), (0, 1), (1, 0), (0, -1)]
    for h in range(H):
        for w in range(W):
            if A[h][w] == "S": Start = (h, w)
            if A[h][w] == "G": Goal = (h, w)
    Q = deque()
    Q.append((Start[0], Start[1], 0, 0)) # h, w, dist, on/off -> o/x
    dp = ["o", "x"]
    while len(Q) > 0:
        ch, cw, dist, door = Q.popleft()
        if D[ch][cw][door] > dist:
            # print(ch, cw, dist, door)
            D[ch][cw][door] = dist
            for dh, dw in M:
                nh = ch + dh
                nw = cw + dw
                if 0 <= nh < H and 0 <= nw < W:
                    ...
                    if a == "#": continue
                        ...
                        if dist + 1 < D[nh][nw][(door + 1) % 2]:
                            Q.append((nh, nw, dist+1, (door+1)%2))
                    else:
                        if dist + 1 < D[nh][nw][door]:
                            if a == "." or a == dp[door] or a == "S" or a == "G":
                                Q.append((nh, nw, dist+1, door))
    # print(D)
    ans = min(D[Goal[0]][Goal[1]])
    if ans == INF: print(-1)
    else: print(ans)

...
def solve():
    ...
    Centor = []
    halfN = N // 2
    def findCenter(idx, pi):
        count = 1
        isCenter = True
        for ne in E[idx]:
            if ne != pi:
                subCount = findCenter(ne, idx)
                if subCount > halfN: isCenter = False
                count += subCount
        if N - count > halfN: isCenter = False
        if isCenter: Centor.append(idx)
        SubTree[idx] = count
        return count
    
    findCenter(0, -1)
    centor = Centor[0]
    for node in E[centor]:
        E[node].discard(centor)
    V = [False] * N
    V[centor] = True
    L = []
    for i in range(N):
            ...
            while len(Q) > 0:
                ...
                if not V[ci]:
                    ...
                    for ni in E[ci]:
                        if not V[ni]:
                            Q.append(ni)
    if N % 2 == 0: L.append(centor)
    for i in range(halfN):
        print(f"{L[i]+1} {L[halfN+i] + 1}")

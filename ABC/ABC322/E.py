...
def solve():
    ...
    for c in range(N):
        ...
        C = l[0]
        A = l[1:]
        for i in reversed(range(maxB + 1)):
            d = DP[i]
            if d < INF:
                BIT = convertToBIT(i, K, P + 1)
                for ai, a in enumerate(A):
                    BIT[ai] = min(P, BIT[ai] + a)
                newIdx = convertBitToInt(BIT, P + 1)
                DP[newIdx] = min(DP[newIdx], d + C)
    if DP[maxB] == INF: print(-1)
    else: print(DP[maxB])

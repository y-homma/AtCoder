...
def solve():
    ...
    Edge = [[None, None] for _ in range(N)] # Red/Blue = 0/1
    for _ in range(M):
    ...
    NonLoop = 0
    for i in range(N):
        if not V[i][0] and not V[i][1]:
            if Edge[i][0] is None and Edge[i][1] is None:
                # 繋がりのないロープ
                NonLoop += 1
                V[i][0] = True
                V[i][1] = True
                ...
                while len(Q) > 0:
                    ...
                    if V[ci][cc]:
                        ...
                        V[ci][cc] = True
                        if ci == pi: # ロープの反対からの移動
                            ne = Edge[ci][cc]
                            if ne is not None:
                                Q.append((ne[0], ne[1], ci))
                        else: # 別のロープからの移動
                            Q.append((ci, (cc + 1) % 2, ci))

                if isLoop: Loop += 1
                else: NonLoop += 1
    print(f"{Loop} {NonLoop}")
    return 0

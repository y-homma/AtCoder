...
def solve():
    ...
    heapq.heapify(pq)
    while len(pq) > 0:
        ccost, cnode, inv = heapq.heappop(pq)
        if ccost < costs[cnode][inv]:
            costs[cnode][inv] = ccost
            for nnode, ninv in edges[cnode]:
                if inv == ninv and ccost + 1 < costs[nnode][ninv]:
                    heapq.heappush(pq, (ccost + 1, nnode, ninv))
            if ccost + X < costs[cnode][(inv + 1) % 2]:
                heapq.heappush(pq, (ccost + X, cnode, (inv + 1) % 2))
    print(min(costs[N-1]))
    return 0

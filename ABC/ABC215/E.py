...
def solve():
    ...
    mod = 998244353 
    A =Alphabet(True)
    L = [A.index(S[i]) for i in range(N)]
    maxB = 2 ** 10
    DP = [[[0] * 10 for u in range(maxB)] for _ in range(N)]
    ...
    for i in range(1, N):
        ...
        for u in range(maxB):
            for j in range(10):
                if si == j:
                    # 使う、使わないの2通り
                    DP[i][u][j] = (DP[i][u][j] + 2 * DP[i-1][u][j]) % mod
                else:
                    # 使わない
                    DP[i][u][j] = (DP[i][u][j] + DP[i-1][u][j]) % mod
                    # まだ使っていない場合は新たに使うことができる
                    if (u & sbit) == 0:
                        combined = u | sbit
                        DP[i][combined][si] = (DP[i][combined][si] + DP[i-1][u][j]) % mod
    
    ans = 0
    for u in range(maxB):
        ans = (ans + sum(DP[N-1][u])) % mod
    print(ans)

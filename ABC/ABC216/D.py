...
def solve():
    ...
    K = [deque() for _ in range(M)]
    Stock = dict()
    q = deque()
    for i in range(M):
        k = int(input())
        K[i] = deque(list(map(int, input().split())))
        ak = K[i].popleft()
        if ak not in Stock.keys():
            Stock[ak] = []
        Stock[ak].append(i)
        if len(Stock[ak]) == 2:
            q.append((ak, Stock[ak]))
            del Stock[ak]

    while len(q) > 0:
        item, indexes = q.popleft()
        for index in indexes:
            if len(K[index]) == 0: continue
            ni = K[index].popleft()
            if ni not in Stock.keys():
                Stock[ni] = [index]
            else:
                Stock[ni].append(index)
                q.append((ni, Stock[ni]))
                del Stock[ni]

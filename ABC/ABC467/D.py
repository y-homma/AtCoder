...
def solve():
    ...
    for t in range(T):
        ...
        Fa = Fraction(qx - px, qy - py)
        Fb = Fraction(sx - rx, sy - ry)
        if Fa == Fb:
            # 重なっていればOK
            midA = ((px + qx) * 0.5, (py + qy) * 0.5)
            midB = ((rx + sx) * 0.5, (ry + sy) * 0.5)
            Fc = Fraction(midB[0] - midA[0], midB[1] - midA[1])
            Fp = Fraction(-(qy-py), qx-px)
            if Fc == Fp:
                Ans[t] = "Yes"
            else:
                Ans[t] = "No"

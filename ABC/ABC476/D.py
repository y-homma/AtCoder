...
def solve():
    ...
    Dessert.sort()
    sumDessert = [0] * (N + 1)
    for i, d in enumerate(Dessert):
        sumDessert[i+1] = d + sumDessert[i]

    ans = bisect.bisect_right(sumDessert, X + K * Y) - 1
    usedK = 0
    change = 0
    Drink.sort()
    # print(Dessert)
    ...
    for i, d in enumerate(Drink):
        ...
        toUse = d // K
        if d % K > 0: toUse += 1
        if toUse + usedK > Y: break
        usedK += toUse
        change += toUse * K - d

        forDrink = X + change + K * (Y - usedK)
        drinkTypes = bisect.bisect_right(sumDessert, forDrink) - 1
        ans = max(ans, drinkTypes + i + 1)
        # print(f"toUse {toUse}, change {change}, forDrink {forDrink}, drinkTypes {drinkTypes}, types {drinkTypes + i + 1}")
